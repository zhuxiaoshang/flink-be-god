...
public class ParquetSchemaConverter {
	 ...
	 */
	public static MessageType toParquetType(TypeInformation<?> typeInformation, boolean legacyMode, String fieldName) {
		return (MessageType) convertField(fieldName, typeInformation, Type.Repetition.OPTIONAL, legacyMode);
	}

	public static MessageType toParquetType(DataType[] dataTypes, boolean legacyMode, String[] fieldNames){
		List<Type> types = new ArrayList<>();
		for(int i = 0; i < dataTypes.length ; i ++) {
			types.add(convertField(fieldNames[i], TypeConversions.fromDataTypeToLegacyInfo(dataTypes[i]), Type.Repetition.OPTIONAL,legacyMode));
		}
