...
public class ParquetSchemaConverter {
    ...
    public static MessageType toParquetType(
            TypeInformation<?> typeInformation, boolean legacyMode, String fieldName) {
        return (MessageType)
                convertField(fieldName, typeInformation, Type.Repetition.OPTIONAL, legacyMode);
    }

    public static MessageType toParquetType(
            DataType[] dataTypes, boolean legacyMode, String[] fieldNames) {
        List<Type> types = new ArrayList<>();
        for (int i = 0; i < dataTypes.length; i++) {
            types.add(
                    convertField(
                            fieldNames[i],
                            TypeConversions.fromDataTypeToLegacyInfo(dataTypes[i]),
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                return Types.primitive(PrimitiveType.PrimitiveTypeName.INT96, repetition)
                        .named(name);
            case MAP:
                return convertToMapType(name, type);
            case ROW:
                return convertToRowType(name, type, repetition);
            case ARRAY:
                return convertToArrayType(name, type);
            default:
     ...
     * @return
     */
    private static Type convertToArrayType(String name, LogicalType type) {
        ArrayType arrayType = (ArrayType) type;
        Type lv3 =
                convertToParquetType(
                        "element", arrayType.getElementType(), Type.Repetition.REQUIRED);
        Type lv2 = Types.repeatedGroup().addFields(lv3).named("list");
        return Types.optionalGroup().addFields(lv2).as(OriginalType.LIST).named(name);
    }

    private static Type convertToRowType(
            String name, LogicalType type, Type.Repetition repetition) {
        RowType rowType = (RowType) type;
        List<String> fieldNames = rowType.getFieldNames();
        Type[] types = new Type[fieldNames.size()];
        for (int i = 0; i < fieldNames.size(); i++) {
            types[i] = convertToParquetType(fieldNames.get(i), rowType.getTypeAt(i), repetition);
        }
        return Types.optionalGroup().addFields(types).named(name);
    }

    /**
     * The outer-most level must be a group annotated with MAP that contains a single field named
     * key_value. The middle level must be a repeated group with a key field for map keys and
     * optionally, a value field for map values. But we can't enforce two strict condition here.
     ...
     * @return
     */
    private static Type convertToMapType(String name, LogicalType type) {
        MapType mapType = (MapType) type;
        LogicalType keyType = mapType.getKeyType();
        LogicalType valueType = mapType.getValueType();
        Type key = convertToParquetType("key", keyType, Type.Repetition.REQUIRED);
        Type value = convertToParquetType("value", valueType);
        GroupType map = Types.repeatedGroup().addFields(key, value).named("key_value");
        return Types.optionalGroup().addFields(map).as(OriginalType.MAP).named(name);
    }
